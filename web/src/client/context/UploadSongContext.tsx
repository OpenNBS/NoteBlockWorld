...
export const UploadSongProvider = ({
  ...
  const submitSongSongData = async (): Promise<string | never> => {
  ...
  const submitSongSongFile = async (id: string): Promise<void | never> => {
    if (!song) return;
  ...
  const submitSong = async () => {
    try {
      const id = await submitSongSongData();
      await submitSongSongFile(id);
    } catch (e) {
      console.error(e);
    }
  };
