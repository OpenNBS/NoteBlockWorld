---
title: 'Attend the Smithed Summit 2026!'
date: '2026-08-08'
author: 'Bentroen'
authorImage: 'Bentroen.png'
image: '/img/blog/summit-26/cover.png'
tags: ['events']
---

**Update (2026-09-01):** Smithed Summit 2026 is now over! Keep an eye out in our blog to find our post-event recap.

---

Hey everyone! **Smithed Summit 2026**, the world's largest Minecraft data pack convention, has just started! Just like last Summit, Note Block World will be at the event, alongside **70+ community projects!**

From **August 8th–22nd, 2026**, you'll be able to:

- find showcase booths;
- attend in-game talks, panels and events;
- hunt for collectibles;
- meet hundreds of players and skilled creators; and
- explore a thriving world jam-packed with secrets to uncover!

All of us vendors have spent **thousands of hours** preparing this event, so we hope to see you there!

Read on to discover some of the things you'll be able to find at the event:

### 🎪 OpenNBS booth

The **Textured Tropics** hosts our booth, two joint buildings which showcase each of our projects: **Note Block Studio** and **Note Block World**.

There, you'll be able to find custom artwork made by the note block community; learn more about the history of each project; and uncover secrets! 🎧

Why not stop for some coffee and delicacies at our cozy _Note Block Café_? ☕

[![OpenNBS booth across a river in the Textured Tropics, with a Note Block World building and a speaker on a riverside table.](/img/blog/summit-26/booth-exterior.png)](/img/blog/summit-26/opennbs-banner.png)
[![Interior of the Note Block World booth, with a glowing Earth-like cube hanging beneath a Note Block World sign.](/img/blog/summit-26/booth-interior.png)](/img/blog/summit-26/opennbs-banner.png)
[![Note Block Café with piston-head tables, cyan and orange chairs, and a snack counter.](/img/blog/summit-26/note-block-cafe.png)](/img/blog/summit-26/opennbs-banner.png)

### 🎶 Music by the Note Block World community

All the **52 music tracks** you hear around Summit's thriving world have been composed by **27 artists** and submitted to Note Block World as part of our [Summit Jam](/blog/summit-jam-2026)!

This time, we have a unique set of tracks playing in each region of the island: the **Patched Plateaus**, the **Textured Tropics**, and the **Welded Woodlands**.

Look for the **music speakers** placed throughout the world! Follow the music, feel the beat, and dance to the rhythm with other visitors! Can you hear **all of the tracks** in each region?

![Wooden music speaker on a birch deck, with a floating blue note particle and water in the background.](/img/blog/summit-26/speaker-woodlands.png)

### 🌐 Note Block World monuments

In each of the regions, you will find our **regional globe monuments** with credits to all the amazing note block artists who crafted the event's soundtrack!

![Three globe monuments: a stone sphere, a yellow moon-like globe, and a steel wireframe globe over a fountain.](/img/blog/summit-26/globe-monuments.png)

### 📔 Sticker Book

Each of the booths featured in the event bring up to five exclusive **stickers** for you to collect. And our own booth, of course, was not left out! Can you complete all of the **four missions** we set up? 🏷

![Open sticker book with rows of uncollected sticker silhouettes labeled by booth name.](/img/blog/summit-26/sticker-book.png)

### 🎷 Figurigamma: An Animated Music Performance

OpenNBS has partnered with [Azagwen](https://azagwen.art/) and [FetchBot](https://x.com/FetchBot1) to bring you an animated, live note block performance, held as an event during the convention!

Powered by Note Block Studio, Blockbench and Animated Java, **Figurigamma** is a 6-minute long, first-of-its-kind exhibition featuring an original note block song played by animated Minecraft instruments.

After the performance, we'll dive into the technical details of how the show was put together. We'll also mention some new features coming to Note Block Studio in the future to assist players in creating their own animated note block projects!

We'll be holding **two exhibitions** of the show:

- 1st exhibition: **Aug 15th 2026, 9:00 PM UTC**
- 2nd exhibition: **Aug 22nd 2026, 1:00 AM UTC**

Don't miss the opportunity to watch this never-before-seen performance live!

[![Figurigamma: an animated music performance. Split banner of clay pots with hanging amethyst crystals, and copper bulbs with orange beacons.](/img/blog/summit-26/figurigamma-banner.png)](/img/blog/summit-26/figurigamma-banner.png)

### ...and much, much more!

All of that is just the beginning! There's so much to explore we cannot cover it all here. So, what are you waiting for? **Go attend Smithed Summit now!** ✨

![Bouncing music speaker in front of a Summit convention hall with a large SUMMIT sign.](/img/blog/summit-26/speaker-plateaus.png)

## How to attend?

Follow these steps to join the event:

### 🎮 Minecraft version and IP

The Summit server will be live at `summit.smithed.net` on **Minecraft: Java Edition** version `26.2`. To join the server, just enter the IP address and connect --- it's that simple!

### 🔧 Suggested mods

Although vanilla clients are fully supported, the Summit team recommends using this [custom modpack](https://modrinth.com/modpack/smithed-summit-26) for the best experience, **especially** if you're planning to join from a weaker computer.

If you are using the Modrinth app to play, you can join using Summit's [server project](https://modrinth.com/server/smithed-summit).

### 📅 Schedule

Throughout the convention, the Summit team will be hosting many events, including informative panel presentations, play sessions, meet & greets, and more.

To view the full schedule and learn more about the event, visit [smithed.net/summit](https://smithed.net/summit)!

All presentations will be streamed live to the Smithed [YouTube channel](youtube.com/@SmithedMC) so you can watch them whenever you want!

![OpenNBS booth at night, with a glowing moon monument, lit Note Block Studio and Note Block World buildings, and a starry sky.](/img/blog/summit-26/booth-exterior-nighttime.png)

## 👥 Acknowledgments

We'd like to sincerely thank the following people, without which our participation in this event could never happen:

- [encode42](http://encode42.dev/): Booth construction; vendoring leadership
- [Vizeon](https://www.youtube.com/@OverlordVizeon): Community management; event soundtrack
- [BuilderAltWCF1](https://builderaltwcf1.carrd.co/): Booth artwork; event soundtrack
- [rmfandyplayz](https://rmfandyplayz.com/): Booth artwork
- [rx97](https://github.com/rx-dev): Playback system optimization
- [Kyrius](https://modrinth.com/user/Kyrius): Globe monuments, props and music speaker models

Figurigamma - Technical staff sheet:

- [erora](https://erora.live/): Music composition
- [Vizeon](https://www.youtube.com/@OverlordVizeon): Note block arrangement
- [Azagwen](https://azagwen.art/): Performance animation
- [FetchBot](https://x.com/FetchBot1): Blockbench plugin development
- [Bentroen](https://youtube.com/@Bentroen_): Playback system development
- [rx97](https://github.com/rx-dev): Playback system optimization
- [SnaveSutit](https://github.com/SnaveSutit): Animated Java consulting
- [JannisX11](https://x.com/JannisX11): Blockbench consulting

Our deepest gratitude goes to:

- [Modrinth](https://modrinth.com/), for supporting the Summit!
- The entire Summit team for allowing us to be part of this whimsical experience!
- The entire note block community. We can only be here because of you!

---

We can't wait to meet you at the event! Have fun!!
